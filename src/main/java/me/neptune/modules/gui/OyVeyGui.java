package me.neptune.modules.gui;

import me.neptune.Neptune;
import me.neptune.modules.Feature;
import me.neptune.modules.gui.items.Item;
import me.neptune.modules.gui.items.buttons.ModuleButton;
...
public class OyVeyGui extends Screen {
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : Neptune.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Neptune.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
