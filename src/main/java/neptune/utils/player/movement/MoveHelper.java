...
public class MoveHelper {
    ...
    public static boolean hasMovement() {
        Vec3d playerMovement = mc.player.getVelocity();
    ...
    public static double getDistanceToGround(Entity entity) {
        ...
        int playerHeight = (int) Math.floor(mc.player.getY());
        double playerZ = mc.player.getZ();

        for (int height = playerHeight; height > 0; height--) {
            BlockPos checkPosition = BlockPos.ofFloored(playerX, height, playerZ);
